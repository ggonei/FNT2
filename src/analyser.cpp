...
const std::string FNT::filePrefix = "/mnt/d/fnt/macros/fntsort/inputs/";
...
FNT::FNT(const char* f, const char* h)	{	//	default constructor
	...
	ULong64_t timeb;	//	initialise time branch variable
	...
	if( gSystem->AccessPathName(filename) ) {	//	if file does not exist
		...
		for( ULong64_t i = 0; i < n; i++ ) {	//	loop over all entries
			...
			if( il == chanb ) setTimeLastB(timeb);	//	update beam time if there is one
		...
		addedTree->Write();	//	Write new tree to file
		tree->Write();	//	Write main tree to file
...
//	histo( 0, "test", 10, 0, 10 );
	...
	newHists->Write();	//	write histograms to file
	return true;	//	success
...
void analyser(){};	//	allow root compilation