...
class FNT {	//	main analysis object
	...
	public:
		...
		TH1D* getH1(std::string s) { return h1s.at(s);/*hs(&h1s,s);*/ }	//	get 1-D histogram
		TH2D* getH2(std::string s) { return h2s.at(s);/*hs(&h2s,s);*/ }	//	get 2-D histogram
		TH3F* getH3(std::string s) { return h3s.at(s);/*hs(&h3s,s);*/ }	//	get 3-D histogram
		unsigned long long getEntries() { return nEntries; }	//	total entries in tree
		...
		short getNumChans() { return (short)channels.size(); }	//	get xpos channel
		void addFriend(TTree* t) { tree->AddFriend(t); };	//	add friend to our tree
		void setMovedFlag(bool m) { movedFlag = m; }	//	set movedFlag flag
		void setTimeOffset(unsigned long long t) { timeOffset = t; }	//	set time offset
