...
public:
	...
	void resetCountdown(){
		percent = -1; countdownN = 1;
	}	//	reset countdown
	void countdown();	//	print progress
	TFitResultPtr peakf( TH1F *h, std::string s );	//	find peaks
//		int getPixelNE(int n = 1) { return ( getPixelE(n) != nonPixel ? getPixelN(n) + n : nonPixel); }	//	get pixel number n rows above and right
	int getPixelN( int pixel, int n = 1 ){
		return pixel + n * 8;
	}	//	get pixel number n rows above
	int getPixelE( int pixel, int n = 1 ){
		return ( ( 8 - ( pixel - 1 ) % 8 ) > n ? pixel + n : nonPixel );
	}	//	get pixel number n after
	int getPixelS( int pixel, int n = 1 ){
		return pixel - n * 8;
	}	//	get pixel number n rows below
	int getPixelW( int pixel, int n = 1 ){
		return ( ( ( pixel - 1 ) % 8 ) > ( n - 1 ) ? pixel - n : nonPixel );
	}	//	get pixel number n before
	bool neighbour( int pixel, int other, int n = 1 ){
		return ( getPixelN( pixel, n ) == other ) + ( getPixelE( pixel, n ) == other ) + ( getPixelS( pixel, n ) == other ) + ( getPixelW( pixel, n ) == other ) + ( ( getPixelE( n ) != nonPixel ? getPixelN( n ) + n : nonPixel ) == other ) + ( ( getPixelE( n ) != nonPixel ? getPixelS( n ) + n : nonPixel ) == other ) + ( ( getPixelW( n ) != nonPixel ? getPixelN( n ) - n : nonPixel ) == other ) + ( ( getPixelW( n ) != nonPixel ? getPixelS( n ) - n : nonPixel ) );
	}	//	get neighbours
