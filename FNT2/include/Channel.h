...
class Channel {	//	object for channels
	...
	public:
		...
		double getPixelNumber() { return pixelNumber; }	//	get pixel number
		char getType() { return type; }	//	get detector type
		int getChannelNumber() { return channelNumber; }	//	get channel number
		...
		double gatesTcover(int bpt) { double s = 0; for( unsigned int i =0; i < gates_t.size(); i++) s += gates_t[i]->percentage(bpt); return s; };
